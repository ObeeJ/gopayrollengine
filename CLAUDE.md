...
- D2C has no payroll relationship to draw eligibility or recourse from. Eligibility must come from `services.PredictNextPayday` (observed bank deposit history via `internal/integrations/banklink`) instead of `AccruedToDate`/`accruedHourlyToDateTx`, and settlement must come from direct debit instead of `SettleAdvancesForPayrollItem` — neither is wired to `EWAAdvance` yet. Don't assume the payroll-funded EWA path applies to a D2C org just because it's still `EWAAdvance`/`WithOrgScope`/the same ledger underneath.
- `banklink.Provider` is deliberately read-only (link + fetch transaction history, nothing else) — no debit capability exists yet. Keep that split when adding one: an account linked for eligibility must never implicitly carry authorization to move money out of it.
