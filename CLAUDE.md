...
- D2C has no payroll relationship to draw eligibility or recourse from. Eligibility should come from `services.PredictNextPayday` (observed bank deposit history via `internal/integrations/banklink`) instead of `AccruedToDate`/`accruedHourlyToDateTx` — **still not wired**: `RequestAdvance` today still runs the payroll-shaped accrual cap against a D2C org, which means nothing for a worker with no payroll relationship. Settlement, unlike eligibility, IS wired: `services.InitiateD2CCollection` + `models.ConfirmD2CCollectionSuccess`/`Failure` (see below), not `SettleAdvancesForPayrollItem`, which stays payroll-triggered and never fires for a D2C org (nothing ever calls `CreatePayroll` for one).
...
- The bank-link-read and debit-mandate-authorization steps (`banklink.Provider.InitiateLink`/`CompleteLink`, `DebitProvider.AuthorizeDebitMandate`) exist at the service layer and are exercised in tests, but nothing in `routes.go` exposes them via HTTP yet — deliberately, since exposing a `Mock`-backed linking flow to real users ahead of a real aggregator doesn't make sense. Don't assume a worker can actually link an account through the API today.
